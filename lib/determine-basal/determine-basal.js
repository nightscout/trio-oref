...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd, tdd_averages) { 
    ...
    var real_autosens_max = Math.max(autosens_min, autosens_max);
    var real_autosens_min = Math.min(autosens_min, autosens_max);
    
    autosens_max = real_autosens_max;
    autosens_min = real_autosens_min;
    
    
    const minLimitChris = Math.min(profile.autosens_min, profile.autosens_max);
    const maxLimitChris = Math.max(profile.autosens.min, profile.autosens_max);
