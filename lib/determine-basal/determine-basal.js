...
// Use minGuardBG to prevent overdosing in hypo-risk situations
    ...
    if (enableSMB && minGuardBG < threshold) {
        ...
        rT.manualBolusErrorString = "Predicted Glucose, " + convert_bg(minGuardBG, profile) + " " + profile.out_units +  ", is below threshold (" + convert_bg(threshold, profile) + ")!";
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (bg < threshold && iob_data.iob < -profile.current_basal*overrideFactor*20/60 && minDelta > 0 && minDelta > expectedDelta) {
     ...
     // predictive low glucose suspend mode: BG is / is projected to be < threshold
        ...
        rT.manualBolusErrorString = "Predicted Minimum Glucose, the minGuardBG, is " + convert_bg(minGuardBG, profile) + " " + profile.out_units + ", which is below Threshold of " +  convert_bg(threshold, profile);
    ...
    if (minDelta < expectedDelta) {
        rT.manualBolusErrorString = "Eventual Glucose > Target Glucose, but glucose is falling faster than expected. Expected: " + expectedDelta + ". Falling: " + minDelta;
        rT.insulinForManualBolus = round((rT.eventualBG - rT.target_bg) / sens, 2);
    ...
    if (Math.min(eventualBG,minPredBG) < max_bg) {
        if (minPredBG < min_bg && eventualBG > min_bg) {
            rT.manualBolusErrorString = "Minimum predicted Glucose is " + convert_bg(minPredBG, profile) + " " + profile.out_units;
            rT.insulinForManualBolus = round((rT.eventualBG - rT.target_bg) / sens, 2);
    ...
    if (iob_data.iob > max_iob) {
        ...
        rT.insulinForManualBolus = round(insulinForManualBolus,3);
