...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd, tdd_averages) { 
    ...
    if (dynISFenabled && tdd > 0) {
        ...
        const isf = profile.sens / newRatio;

        sigmoidLog = ". Using Sigmoid function, the autosens ratio has been adjusted with sigmoid factor to: " + round(autosens_data.ratio, 2) + ". New ISF = " + round(isf, 2) + " mg/dl (" + round(0.0555 * isf, 2) + " (mmol/l)" + ". CR adjusted from " + round(cr,2) + " to " + round(profile.carb_ratio,2) + " (" + round(0.0555 * profile.carb_ratio, 2) + " mmol/l).";
...
// Adjust ISF based on sensitivityRatio
    ...
    // Temporary fix/test
    if (typeof(iob_data.length) && iob_data.length > 1 && tdd_before > 0) {
        iob_data = iobArray[0];
    } 
...
// Initialize and calculate variables used for predicting BGs
    ...
    for (var i=IOBpredBGs.length-1; i > 12; i--) {
        ...
        if (IOBpredBGs[i-1] !== IOBpredBGs[i] && !i < 0) { break; }
        else { IOBpredBGs.pop(); }
    }
    ...
    for (i=ZTpredBGs.length-1; i > 6; i--) {
        ...
        if (ZTpredBGs[i-1] >= ZTpredBGs[i] || ZTpredBGs[i] <= target_bg && !i < 0) { break; }
        else { ZTpredBGs.pop(); }
    }
    ...
    if (meal_data.mealCOB > 0 && ( ci > 0 || remainingCIpeak > 0 )) {
        ...
        for (i=COBpredBGs.length-1; i > 12; i--) {
            if (COBpredBGs[i-1] !== COBpredBGs[i] && !i < 0) { break; }
            else { COBpredBGs.pop(); }
        }
    ...
    if (ci > 0 || remainingCIpeak > 0) {
        if (enableUAM) {
            ...
            for (i=UAMpredBGs.length-1; i > 12; i--) {
                if (UAMpredBGs[i-1] !== UAMpredBGs[i] && !i < 0) { break; }
                else { UAMpredBGs.pop(); }
            }
