...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd, tdd_averages) { 
    ...
    if (!chrisFormula && !useDynamicCR) {
        tddReason += "";
    } else if (chrisFormula && profile.tddAdjBasal) { 
        tddReason += log_isfCR + formula + limitLog + afLog + basal_ratio_log; 
    }
    else if (chrisFormula && !profile.tddAdjBasal) { tddReason += log_isfCR + formula + limitLog + afLog; }
...
//    var sensitivityRatio;
    ...
    if (profile.temptargetSet && target_bg < normalTarget && profile.enableChris && BG >= target_bg) {
        if (sensitivityRatio < newRatio) {
            sensitivityRatio = newRatio * (normalTarget/target_bg);
            //Use autosesns.max limit
            Math.min(sensitivityRatio, profile.autosens_max);
            console.log("Dynamic ratio increased from " + round(newRatio, 2) + " to " + round(sensitivityRatio,2) + " due to a low temp target (" + target_bg + ").");
        }
    }
    ...
    if (sensitivityRatio && profile.enableChris == false) { // Disable adjustment of basal by sensitivityRatio when using dISF
    ...
    else if (profile.enableChris && profile.tddAdjBasal) {
        ...
        if (basal !== profile_current_basal) {
            process.stderr.write("Adjusting basal from " + profile_current_basal + " U/h to " + basal + " U/h; ");
        } else {
            process.stderr.write("Basal unchanged: " + basal + " U/h; ");
        }
    }
