...
function enable_smb(profile, microBolusAllowed, meal_data, bg, target_bg, high_bg, oref_variables, time) {
    if (oref_variables.smbIsAlwaysOff){
        /* If smbIsAlwaysOff is Schedule When SMBs are Off, so if it is true, we have a time frame to disable SMBs
...
// Initialize and calculate variables used for predicting BGs
    ...
    // smbIsOff is DisableSMBs, if true, disable SMBs
    // smbIsAlwaysOff is Schedule when SMBs are off, if false, we do not have a schedule, so disable them
    if (smbIsOff && !smbIsAlwaysOff) {
