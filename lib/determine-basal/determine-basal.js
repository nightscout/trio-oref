...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, oref2_variables) { 
    ...
    if (pumpData > 21) {
        ...
        tddReason = ", Total insulin: " + round(tdd,2) + " U, " + round(bolusInsulin/tdd*100,0) + "% Bolus " + round((tempInsulin+scheduledBasalInsulin)/tdd*100,0) +  "% Basal";
    ...
    if (dynISFenabled && enable_sigmoid) {  
        ...
        const bg_dev = (BG - profile.min_bg) * 0.0555;
        ...
        tddFactorLog = ", tdd_factor: " + round(tdd_factor, 1);
        const max_minus_one = maxLimitChris - 1;
        ...
        // Dynamic CR will be done further down
    }
    ...
    if (dynISFenabled && tdd > 0) {
        ...
        sigmoidLog = ". Using Sigmoid function, the autosens ratio has been adjusted with sigmoid factor to: " + round(autosens_data.ratio, 2) + ". New ISF = " + round(isf, 2) + " mg/dl (" + round(0.0555 * isf, 2) + " (mmol/l)" + ". CR adjusted from " + round(cr_before,2) + " to " + round(cr,2) + " (" + round(0.0555 * carbRatio, 2) + " mmol/l).";
    ...
    if (oref2_variables.useOverride) {  
        if (oref2_variables.duration == 0) {
            console.log("Profile Override is active. Override " + overrideFactor * 100 + "%. Oerride Duration: " + "Enabled indefinitely");
        } else 
            console.log("Profile Override is active. Override " + overrideFactor * 100 + "%. Override Expires in: " + oref2_variables.duration + " min.");
    }
...
// Print summary variables based on predBGs etc.
    ...
    var targetLog = rT.target_bg;
    if  (target_bg != profileTarget) {
        targetLog = convert_bg(profileTarget, profile) + "\u2192" + rT.target_bg;
    }
    ...
    rT.reason += tddReason + tddFactorLog;
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            if (typeof profile.maxSMBBasalMinutes === 'undefined' ) {
                ...
                if (profile.maxUAMSMBBasalMinutes) {
                    console.error("profile.maxUAMSMBBasalMinutes: " + profile.maxUAMSMBBasalMinutes + ", profile.current_basal: " + profile.current_basal * overrideFactor);
                    maxBolus = round(profile.current_basal * overrideFactor * profile.maxUAMSMBBasalMinutes / 60 ,1);
                } else {
                ...
                console.error("profile.maxSMBBasalMinutes: " + profile.maxSMBBasalMinutes + ", profile.current_basal: " + profile.current_basal * overrideFactor);
