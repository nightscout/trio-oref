...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd, tdd_averages) { 
    ...
    // Check for empty pump history. If empty: don't use dynamic settings!
    ...
    if (tdd24h_7d_Ratio > 1) {
        ...
        console.log("Basal adjustment with a 24 hour to 7 day average TDD ratio (limited by Autosens max setting). Basal Ratio: " + tdd24h_7d_Ratio + ". Upper limit = Autosens max (" + profile.autosens_max + ")");
    }
    else if (tdd24h_7d_Ratio < 1) {
        tdd24h_7d_Ratio = Math.max(tdd24h_7d_Ratio, profile.autosens_min);
        tdd24h_7d_Ratio = round(tdd24h_7d_Ratio,2);
        console.log("Basal adjustment with a 24 hour to 7 day average TDD ratio (limited by Autosens min setting). Basal Ratio: " + tdd24h_7d_Ratio + ". Lower limit = Autosens min (" + profile.autosens_min + ")");
    } 
    else { 
...
// Initialize and calculate variables used for predicting BGs
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            // Adding dynamic ISF in predictions for ZT and IOB. Modification from Tim Street's AAPS but with addded Original Formula and default as off:
            switch(true) {
                case chrisFormula == true && preferences.useNewFormula == true:
                    ...
                    break;
                case chrisFormula == true && preferences.useNewFormula == false:
                    IOBpredBG = IOBpredBGs[IOBpredBGs.length-1] + (round( -iobTick.activity * (277700 / ( tdd * adjustmentFactor * Math.max( IOBpredBGs[IOBpredBGs.length-1],39))) * 5 ),2) + predDev;
                    var ZTpredBG = ZTpredBGs[ZTpredBGs.length-1] + (round( -iobTick.iobWithZeroTemp.activity * (277700 / ( tdd * adjustmentFactor * Math.max(ZTpredBGs[ZTpredBGs.length-1],39))) * 5 ), 2);       
                    console.log("Dynamic ISF (Original Formula) adjusted predictions for IOB and ZT: IOBpredBG: " + round(IOBpredBG,2) + " , ZTpredBG: " + round(ZTpredBG,2));
                    break;
                default:
            ...
            // Adding dynamic ISF in predictions for UAM. Modification from Tim Street's AAPS but with addded Original Formula and default as off:
            switch(true) {
                case chrisFormula == true && preferences.useNewFormula == true:
                    ...
                    console.log("Dynamic ISF (Logarithmic Formula) adjusted predictions for UAM: UAMpredBG: " + round(UAMpredBG,2));
                    break;
                case chrisFormula == true && preferences.useNewFormula == false:
                    UAMpredBG = UAMpredBGs[UAMpredBGs.length-1] + (round( -iobTick.activity * (277700 / ( tdd * adjustmentFactor * Math.max(UAMpredBGs[UAMpredBGs.length-1],39))) * 5 ),2) + Math.min(0, predDev) + predUCI;
                    console.log("Dynamic ISF (Original Formula) adjusted predictions for UAM: UAMpredBG: " + round(UAMpredBG,2));
                    break;
                default:
