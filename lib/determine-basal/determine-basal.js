...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd, tdd_averages) { 
    ...
    if (pumpData > 21) {
        ...
        if (weightedAverage > 0 && average_total_data > 0) {
            tddReason = ", TDD: " + round(tdd,1) + " U, Weighted avg: " + round(weightedAverage,1) + " U, Total data avg: " + round(average_total_data,1) + " U";
    ...
    // respect autosens_max/min fortdd24h_14d_Ratio, used to adjust basal similarly as autosens
    ...
    tdd24h_14d_Ratio = round(tdd24h_14d_Ratio,2);
        console.log("Basal adjustment with a 24 hour to 7 day average TDD ratio (limited by Autosens max setting). Basal Ratio: " + tdd24h_14d_Ratio + ". Upper limit = Autosens max (" + profile.autosens_max + ")");
    }
    else if (tdd24h_14d_Ratio < 1) {
    tdd24h_14d_Ratio = Math.max(tdd24h_14d_Ratio, profile.autosens_min);
    tdd24h_14d_Ratio = round(tdd24h_14d_Ratio,2);
        console.log("Basal adjustment with a 24 hour to 7 day average TDD ratio (limited by Autosens min setting). Basal Ratio: " + tdd24h_14d_Ratio + ". Lower limit = Autosens min (" + profile.autosens_min + ")");
    } 
    else { 
        console.log("Basal adjusted with a 24 hour to 7 day average TDD ratio: " + tdd24h_14d_Ratio);
    }

    var basal_ratio_log = ", Basal ratio: " + tdd24h_14d_Ratio;
    ...
    if (chrisFormula) {
        ...
        console.log("autoISF is off. dynISF is on");
    }
    ...
    if (chrisFormula == false && useDynamicCR == false) {
        tddReason += "";
    } else { tddReason += log_isfCR + formula + limitLog + afLog + basal_ratio_log; }

    // --------------- END OF DYNAMIC RATIOS CALCULATION  ------ A FEW LINES ADDED ALSO AT LINE NR 1374 and 1415 ------------------------------------------------
...
//    var sensitivityRatio;
    ...
    if (sensitivityRatio && profile.enableChris == false) { // Disable adjustment of basal by sensitivityRatio when using dISF
        ...
        basal = profile.current_basal * tdd24h_14d_Ratio;
        basal = round_basal(basal, profile);
        process.stderr.write("TDD-adjustment of basals activated, using tdd24h_14d_Ratio " + round(tdd24h_14d_Ratio,2) + ", TDD 24h = " + round(tdd_before,2) + "U, Weighted average TDD = " + round(weightedAverage,2) + "U, (Weight percentage = " + weightPercentage + "), 7-day average TDD = " + round(average_total_data,2) + "U. " );
...
// Conversely, adjust BG target based on autosens ratio if no temp target is running
    ...
    var threshold_setting = profile.threshold_setting;
    if (120 >= threshold_setting >= 65 && threshold_setting > threshold) {
        console.log("Threshold changed in settings from " + convert_bg(threshold, profile) + " to " + convert_bg(threshold_setting, profile) + ". ");
