...
// Conversely, adjust BG target based on autosens ratio if no temp target is running
    ...
    // Set threshold to the user's setting, as long as it's between 65-120 and above the default calculated threshold
    threshold = Math.min(Math.max(profile.threshold_setting, threshold, 65), 120);
...
// Initialize and calculate variables used for predicting BGs
    ...
    if (smbIsOff) {
        if (!smbIsAlwaysOff) {
            console.error("SMBs are disabled by profile override");
            enableSMB = false;
        } else {
            let hour = currentTime.getHours();
            if (end < start && hour < 24 && hour > start) {
                end += 24;
            }
            if (hour >= start && hour <= end) {
                console.error("SMB disabled by profile override");
                enableSMB = false;
            }

            if (end < start && hour < end) {
                console.error("SMB disabled by profile override");
                enableSMB = false;
            }
        }
